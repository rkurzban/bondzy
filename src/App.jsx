...
const Create = ({onNav,email,onCreate,form:f,setForm:sF}) => {
  ...
  const v3 = () => { const e={}; if(!f.rewardLink.trim())e.rewardLink="Required"; if(!f.rewardDescription.trim())e.rewardDescription="Required"; setErr(e); return !Object.keys(e).length; };
  const sub = () => { if(!v3()) return; onCreate({id:genId(),type:"reward",creatorEmail:email,recipientName:f.recipientName,recipientEmail:f.recipientEmail,locationName:f.locationName,locationAddress:f.locationAddress,locationLat:f.locationLat,locationLng:f.locationLng,date:f.date,time:f.time,graceMinutes:parseInt(f.graceMinutes)||10,rewardLink:f.rewardLink,rewardDescription:f.rewardDescription,status:"active",createdAt:new Date().toISOString()}); };
  ...
  return <div style={{maxWidth:540,margin:"0 auto",padding:"28px 20px 80px"}}>
    ...
    <div className="crd" style={{animation:"fadeIn 0.3s ease"}}>
      ...
      {step===2&&<>
        ...
        <div style={fw}><label style={ls}>Grace Period</label><select className="inp" value={f.graceMinutes} onChange={e=>up("graceMinutes",e.target.value)}><option value="5">5 minutes</option><option value="10">10 minutes</option><option value="15">15 minutes</option><option value="30">30 minutes</option><option value="60">1 hour</option></select></div>
      ...
      {step===3&&<>
        ...
        <div style={fw}><label style={ls}>Describe the Reward</label>{er(err.rewardDescription)}<textarea className="inp" placeholder="e.g. $25 PayPal for hitting the gym!" value={f.rewardDescription} onChange={e=>up("rewardDescription",e.target.value)} style={{minHeight:70,resize:"vertical"}}/></div>
        <div style={{background:B.off,borderRadius:10,padding:16,marginBottom:20,fontSize:13,lineHeight:1.7,border:`1px solid ${B.bdr}`}}>
          ...
          <strong>{f.recipientName||"___"}</strong> must be at <strong>{f.locationName||"___"}</strong> on <strong>{f.date?fmtD(f.date):"___"}</strong> at <strong>{f.time?fmtT(f.time):"___"}</strong> (±{f.graceMinutes} min) to claim: <strong>{f.rewardDescription||"___"}</strong>
        </div>
